  ...
  return (
    <div className="flex flex-col gap-8 lg:gap-10">
      <div className="space-y-6">
        <h1 className="max-w-xl text-[2rem] leading-[1.15] font-bold tracking-tight text-white sm:text-[2.5rem] lg:text-[3.25rem] lg:leading-[1.1]">
          {content.title}
        </h1>
        ...
        {content.features.length > 0 ? (
          <ul className="flex max-w-xl flex-col gap-3.5 sm:gap-4">
            {content.features.map((feature, index) => (
              <FeatureItem key={`${feature}-${index}`} text={feature} showInfo={index === 1} />
            ))}
          </ul>
        ) : null}
      </div>
      ...
      <div className="flex flex-col gap-5 sm:gap-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
          ...
          {content.secondaryButtonText ? (
            <HeroButton href={content.secondaryButtonLink || '#'} variant="secondary">
              {content.secondaryButtonText}
            </HeroButton>
          ) : null}
        </div>
